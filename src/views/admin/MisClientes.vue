...
cambiarEstadoCliente(cliente) {
  ...
  const confirmacion = confirm(`¿Está seguro que desea ${accion} al cliente ${cliente.nombreEmpresa}?`);
  
  if (confirmacion) {
    cliente.estado = cliente.estado === 'activo' ? 'inactivo' : 'activo';
    alert(`Cliente ${accion === 'desactivar' ? 'desactivado' : 'activado'} exitosamente`);
  }
},
