...
aplicarPrioridadEstados(cotizaciones) {
 return cotizaciones.sort((a, b) => {
   // 🆕 PRIORIDAD FIJA DE ESTADOS
   const prioridades = {
     'pendiente_aprobacion': 1, // MÁXIMA PRIORIDAD
     'pendiente': 2,
     'efectiva': 3,
     'rechazada': 4
   };
...
async confirmarAprobacion(cotizacion) {
 try {
   const response = await this.cotizacionService.aprobarCotizacion(cotizacion.id);
...
// ACTUALIZADO: Ahora abre el modal de confirmación
...
// NUEVO: Método para cerrar el modal de confirmación efectiva
...
getEstadoClase(estado) {
 // Retornar directamente el estado para que coincida con las clases CSS
 return estado;
},
